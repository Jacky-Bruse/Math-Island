...
- 加减法/比大小在到点提醒点击“继续”后进入“收尾阶段”，完成追加题后自动结束本次训练并返回首页
...
- 加减法/比大小在 `Continue` 完成追加题后进入 `Ending`
...
- 每盘必须可解
- 4×4 与 6×6需唯一解
- 8×8目标唯一解率 >= 95%（按发布前抽样题库统计）
- 在线生成超时判定口径：从发起生成请求到产出可用 `givens + solution` 并完成必要校验（可解/唯一解规则）的总耗时
- 在线生成超时阈值默认 1500ms，支持按尺寸配置（4×4 / 6×6 / 8×8）
- 超时后回退到内置预生成题库（均为唯一解）
...
- 默认不保存逐题历史
- 保存最近 7 天聚合指标（本地）：答题总数、正确率、完成盘数，仅用于本地展示
- 出题与难度分布不读取历史指标，按固定规则执行
- 设置中提供“一键清空本地学习数据”
- 数独盘面可选保存未完成草稿以支持恢复
...
- 加减法/比大小在到点提醒点击“继续”后严格追加 3 题并自动结束
