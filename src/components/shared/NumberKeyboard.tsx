...
export default function NumberKeyboard({ onInput, onBackspace, onClear, onConfirm, disabled }: Props) {
  const btnClass = 'w-full min-h-[clamp(3.4rem,7.8vh,4.6rem)] rounded-2xl bg-surface shadow-sm text-[clamp(1.2rem,2.7vw,1.9rem)] font-bold active:scale-95 transition-transform disabled:opacity-40'
  ...
  return (
    <div className="grid grid-cols-4 gap-2.5 md:gap-3 p-2 md:p-3 max-w-3xl mx-auto">
      ...
      <button className={`${btnClass} bg-surface-muted text-[clamp(1rem,2.1vw,1.3rem)]`} onClick={onBackspace} disabled={disabled}>
        ⌫
      </button>
      {[4, 5, 6].map(n => (
        <button key={n} className={btnClass} onClick={() => onInput(n)} disabled={disabled}>{n}</button>
      ))}
      <button className={`${btnClass} bg-surface-muted text-[clamp(0.95rem,2vw,1.2rem)]`} onClick={onClear} disabled={disabled}>
        清空
      </button>
      {[7, 8, 9].map(n => (
        <button key={n} className={btnClass} onClick={() => onInput(n)} disabled={disabled}>{n}</button>
